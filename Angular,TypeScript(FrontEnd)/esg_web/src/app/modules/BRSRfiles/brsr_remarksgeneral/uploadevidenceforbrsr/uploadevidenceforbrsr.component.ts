...
@Component({
  selector: 'app-uploadevidenceforbrsr',
  templateUrl: './uploadevidenceforbrsr.component.html',
  styleUrls: ['./uploadevidenceforbrsr.component.scss']
})
export class UploadevidenceforbrsrComponent implements OnInit {
  reportid: any;
  ...
  constructor(
      ...
      private is: SelectbrsrService,
      private router: Router,
      public matDialogRef: MatDialogRef<UploadevidenceforbrsrComponent>,
  ...
  getReportEvidence(ReportId, Ques) {
      this.uploadService
          .GetBRSRUploadEvidenceByReportId(ReportId, Ques)
          .subscribe((data: any) => {
              console.log(data);
              if (data.length > 0) {
  ...
  uploadFiles() {
      ...
      for (let i = 0; i < this.docArr.length; i++) {
          ...
          console.log(obj);
          this.uploadService.createBRSRImages(obj).subscribe((res) => {
              console.log(res);
          });
