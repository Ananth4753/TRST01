...
@Component({
  selector: 'app-scope3category5',
  templateUrl: './scope3category5.component.html',
  styleUrls: ['./scope3category5.component.css']
})
export class Scope3category5Component implements OnInit {
  lookupdtl:any;
  material:any;
  newAttribute:any
  sumtotal:any;
  ...
  constructor(public dialog: MatDialog,private fb:FormBuilder,private is:ImportdisService,private cdRef: ChangeDetectorRef,
    @Inject(MAT_DIALOG_DATA) private _data: { data },private router:Router,public matDialogRef:MatDialogRef<Scope3category5Component>) { 
    this.lookupdtl=[];
  }
  ngOnInit() {
    ...
    this.is.GetScope3Category5WasteMaterialNames().subscribe((data)=>{
      this.materialnames=data;
    })
    this.addFieldValue();
  ...
  materialdropdown(id: any, text: any, i: number) {
    console.log(id); // Selected ID
    console.log(text); 
    this.is.GetScope3Category5WasteById(text).subscribe((data)=>{
      console.log(data);
      this.material=data
    })
  }
  disposalchange(value: any, index: number) {
    ...
    if (selectedGood != undefined) {
      this.lookupdtl[index].standardef = selectedGood.Standardef;
    }
...
this.is.scope3category5=Math.round(this.finalesum)
