...
@Component({
  selector: 'app-scope3category11',
  templateUrl: './scope3category11.component.html',
  styleUrls: ['./scope3category11.component.scss']
})
export class Scope3category11Component implements OnInit {
  lookupdtl:any;
  lookupdtl1:any;
  lookupdtl2:any;
  lookupdtl3:any;
  ...
  constructor(public dialog: MatDialog,private fb:FormBuilder,private is:ImportdisService,private cdRef: ChangeDetectorRef,
    @Inject(MAT_DIALOG_DATA) private _data: { data },private router:Router,public matDialogRef:MatDialogRef<Scope3category11Component>) { 
    this.lookupdtl=[];
    this.lookupdtl1=[];
    this.lookupdtl2=[];
    this.lookupdtl3=[];
  ...
  calculateTotalEmission2() {
    ...
    for (let i = 0; i < this.lookupdtl2.length; i++) {
      totalEmission += parseFloat(this.lookupdtl2[i].seven);
    }
  ...
  calculateTotalEmission3() {
    ...
    for (let i = 0; i < this.lookupdtl3.length; i++) {
      totalEmission += parseFloat(this.lookupdtl3[i].uno4);
    }
    this.sumtotal3 = totalEmission.toFixed(2);
    this.finalesum = this.sumtotal3;
  }
  qu(index: number, event: any){
    this.lookupdtl[index].fifth=event.target.value
    const quant1 = this.lookupdtl[index].second;
    const quant2 = this.lookupdtl[index].third;
    const quant3= this.lookupdtl[index].fourth;
    const quant4=this.lookupdtl[index].fifth
    var saver
    ...
    this.lookupdtl[index].sixth = ((saver * saver1 * saver2 * saver3)/1000).toFixed(2);
    this.calculateTotalEmission();
  }
  a2ndqu(index: number, event: any){
    const quant4 = event.target.value;
    const quant1 = this.lookupdtl1[index].uno1;
    var saver
    saver=quant1.replace(/\,/g,''); // 1125, but a string, so convert it to number
    saver=parseFloat(saver);
    var saver3
  ...
  thrdca(index: number, event: any){
    ...
    const quant2 = this.lookupdtl2[index].third;
    const quant3 = this.lookupdtl2[index].fourth;
    const quant4 = this.lookupdtl2[index].five;
    var saver
    saver=quant1.replace(/\,/g,''); // 1125, but a string, so convert it to number
    saver=parseFloat(saver);
    var saver1
    ...
    saver3=parseFloat(saver3);
    var saver5
    saver5=quant5.replace(/\,/g,''); // 1125, but a string, so convert it to number
    saver5=parseFloat(saver5);
    this.lookupdtl2[index].seven = ((saver*(saver1/100)*saver2*saver3*saver5)/1000).toFixed(2) ;
    this.calculateTotalEmission2();
  }
  last(index: number, event: any){
    const quant3 = event.target.value;
    const quant1 = this.lookupdtl3[index].uno1;
    const quant2 = this.lookupdtl3[index].uno2;
    var saver1
    ...
    saver3=parseFloat(saver3);
    this.lookupdtl3[index].uno4 = ((saver1*saver3*saver2)/1000).toFixed(2) ;
...
this.is.scope3category11=Math.round(this.finalesum)
