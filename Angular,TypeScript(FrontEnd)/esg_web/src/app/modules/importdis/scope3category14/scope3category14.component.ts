...
@Component({
  selector: 'app-scope3category14',
  templateUrl: './scope3category14.component.html',
  styleUrls: ['./scope3category14.component.scss']
})
export class Scope3category14Component implements OnInit {
  lookupdtl:any;
  lookupdtl1:any;
  lookupdtl2:any;
  lookupdtl3:any;
  ...
  constructor(public dialog: MatDialog,private fb:FormBuilder,private is:ImportdisService,private cdRef: ChangeDetectorRef,
    @Inject(MAT_DIALOG_DATA) private _data: { data },private router:Router,public matDialogRef:MatDialogRef<Scope3category14Component>) { 
    this.lookupdtl=[];
    this.lookupdtl1=[];
    this.lookupdtl2=[];
    this.lookupdtl3=[];
  ...
  calculateTotalEmission1() {
    ...
    for (let i = 0; i < this.lookupdtl1.length; i++) {
      totalEmission += parseFloat(this.lookupdtl1[i].uno5);
    }
  ...
  calculateTotalEmission2() {
    ...
    for (let i = 0; i < this.lookupdtl2.length; i++) {
      totalEmission += parseFloat(this.lookupdtl2[i].four);
    }
  ...
  calculateTotalEmission3() {
    ...
    for (let i = 0; i < this.lookupdtl3.length; i++) {
      totalEmission += parseFloat(this.lookupdtl3[i].uno3);
    }
  ...
  firstcal(index: number, event: any){
    ...
    this.lookupdtl[index].fourth = (saver + saver1).toFixed(2);
  ...
  secondcal(index: number, event: any){
    ...
    const c = this.lookupdtl1[index].uno3;
    var saver
    ...
    const y=saver1*(saver2/100)
    this.lookupdtl1[index].uno5 = ((saver/y)*saver3 ).toFixed(2);
  ...
  thrdca(index: number, event: any){
    ...
    saver1=parseFloat(saver1);
    this.lookupdtl2[index].four = ((saver*saver1)/1000).toFixed(2) ;
  ...
  last(index: number, event: any){
    ...
    saver3=parseFloat(saver3);
    this.lookupdtl3[index].uno3 = ((saver1*saver3)/1000).toFixed(2) ;
...
this.is.scope3category14=Math.round(this.finalesum)
